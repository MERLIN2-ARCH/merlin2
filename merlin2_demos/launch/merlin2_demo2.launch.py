...
def generate_launch_description():
    ...
    total_points_cmd = DeclareLaunchArgument(
        "total_points", default_value="6", description="Total points"
    )
    ...
    number_of_tests_cmd = DeclareLaunchArgument(
        "number_of_tests", default_value="5", description="Number of tests"
    )

    results_path = LaunchConfiguration("results_path")
    results_path_cmd = DeclareLaunchArgument(
        "results_path", default_value="~/", description="Path to store the results"
    )

    world = LaunchConfiguration("world")
    world_cmd = DeclareLaunchArgument(
        "world", default_value="granny", description="World used in tests"
    )
    ...
    waypoint_navigation_cmd = IncludeLaunchDescription(
        ...
        launch_arguments={
            "wps": ament_index_python.get_package_share_directory("merlin2_demos")
            + "/params/granny.yaml"
        }.items(),
    )
