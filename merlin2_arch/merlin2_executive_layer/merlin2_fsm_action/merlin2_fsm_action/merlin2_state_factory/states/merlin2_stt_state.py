...
class Merlin2SttState(StateMachine):
    ...
    def __init__(self, node: Node) -> None:
        ...
        checking_speech_state = CbState(["valid", "repeat"], self.checking_speech)

        calibrating_state = ServiceState(
            Empty,
            "/speech_to_text/calibrate_listening",
            self.create_calibrate_request,
            node=node,
        )

        tts_state = ActionState(
            TTS, "/text_to_speech/tts", self.create_tts_goal, node=node
        )

        stt_state = ActionState(
            ListenOnce,
            "/speech_to_text/listen_once",
            self.create_stt_goal,
            result_handler=self.result_stt_handler,
            node=node,
        )

        self.add_state("CALIBRATING", calibrating_state, {SUCCEED: "LISTENING"})
        ...
        self.add_state(
            ...
            {"repeat": "COMPLAINING", "valid": SUCCEED},
        )

        self.add_state("COMPLAINING", tts_state, {SUCCEED: "CALIBRATING"})
    ...
    def result_stt_handler(
        self, blackboard: Blackboard, result: ListenOnce.Result
    ) -> str:
        """handle STT results
