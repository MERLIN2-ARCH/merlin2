...
from kant_dto import PddlPropositionDto, PddlConditionEffectDto, PddlObjectDto
from merlin2_msgs.action import DispatchPlan, DispatchAction
...
class Merlin2PlanDispatcherNode(Node):
    ...
    def __execute_server(self, goal_handle) -> DispatchPlan.Result:
        ...
        for action in goal_handle.request.plan:
            ...
            for object_name, pddl_parameter_dto in zip(
                action.objects, pddl_parameter_dto_list
            ):
                pddl_object_dto = self.pddl_object_dao.get(object_name)
                pddl_objects_dto_dict[pddl_parameter_dto.get_name()] = pddl_object_dto
            ...
            if pddl_action_dto.get_durative():
                ...
                for pddl_effect_dto in pddl_efect_dto_list:
                    ...
                    pddl_effect_dict[pddl_effect_dto.get_time()].append(pddl_effect_dto)
    ...
    def __update_knowledge(
        self, pddl_proposition_dto: PddlPropositionDto, deleted: bool
    ) -> bool:
        ...
        if deleted:
            succeed = self.pddl_proposition_dao.delete(pddl_proposition_dto)
        else:
